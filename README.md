...
### Selenium / browser integration tests (Failsafe ITs)

- [ ] **Retry the whole test on any `WebDriverException` — and know that a timeout _is_ one.** The dominant flake in a multi-browser IT suite is not a code bug, it's a timing timeout: a 3-player test drives 3 Chromes against one Spring server, and under CI load a `WebDriverWait` that normally settles in 1–2 s blows past an 8 s ceiling, failing the build on a run that passes clean on retry. Add a JUnit 5 `TestExecutionExceptionHandler` extension that re-runs the test method (re-invoking `@BeforeEach` so a dead driver is recreated) up to N times on `WebDriverException`. **The load-bearing detail: `org.openqa.selenium.TimeoutException extends WebDriverException`,** so a single handler catches both a genuinely-crashed session (`NoSuchSessionException`) _and_ these load-induced `WebDriverWait` timeouts. **The trap that actually bit us:** the retry is opt-in per method (`@ExtendWith(...)` + a marker annotation), so one test that _forgot_ the annotation is the one that reddens CI — grep that every browser test carries it, or move the extension to the class/`@ExtendWith` on the base so it can't be missed.
- [ ] **Don't chase a driver/browser version skew that doesn't exist.** It's tempting to blame flakes on the cached chromedriver lagging the auto-updating Chrome (`150.0.7871.124` driver vs `.182` browser). It's a red herring: **chromedriver is compatible across patch versions within the same major** — a `150.x` driver drives any `150.x` Chrome. `SessionNotCreated` only fires on a _major_ mismatch. Setting `options.setBrowserVersion("stable")` (so Selenium Manager resolves a Chrome-for-Testing pair) is harmless and buys hermeticity from the dev's own Chrome profile, but it will **not** fix a timing flake — measure before attributing a flake to versions, or you'll ship a no-op fix and the red stays red.
- [ ] **Reap the per-instance `--user-data-dir` profiles.** A unique profile dir per driver (`/tmp/chrome-user-data-<nanoTime>`) avoids cross-instance conflicts when ITs run in parallel, but nothing deletes it on `quit()` — they silently accumulate into _thousands_ of stale dirs (2 700+ observed on one machine, minutes to `rm`). Add `rm -rf /tmp/chrome-user-data-* 2>/dev/null || true` to the pre-test/deploy cleanup alongside the process kill, or delete the dir in driver teardown at the source.
- [ ] **Scope the process cleanup to test-only names.** `pkill -f chromedriver` + `pkill -f "Chrome for Testing"` reaps orphaned drivers/browsers from a crashed run without touching the developer's real Chrome (a second, minor reason to prefer CfT: orphaned test browsers become safe to kill by name).
